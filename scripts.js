            ...
            if (value.photos && value.photos.length > 0) {
                value.photos.forEach((photo, index) => {
                    ...
                    if (Array.isArray(value.aiAnalysis) && value.aiAnalysis[index]) {
                        ...
                        doc.text(`Status: ${aiComment.status || 'unknown'}`, 75, y - 30);

                        if (Array.isArray(aiComment.issues) && aiComment.issues.length > 0) {
    ...
    try {
        const responses = await Promise.all(
		    ...
		    console.log('Payload enviado al backend:', {
                ...
                if (!response.ok) {
                    console.error(`HTTP error for image ${index + 1}:`, response.status);
                    throw new Error(`HTTP error: ${response.status}`);
                }
